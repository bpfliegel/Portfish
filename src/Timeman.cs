...
namespace Portfish
{
    internal static class TimeMgr
    {
        internal static int optimumSearchTime;
        internal static int maximumSearchTime;
        internal static int unstablePVExtraTime;

        /// Constants
        internal const int MoveHorizon = 50;    // Plan time management at most this many moves ahead
        internal const float MaxRatio = 3.0f;  // When in trouble, we can step over reserved time with this ratio
        internal const float StealRatio = 0.33f; // However we must not steal time from remaining moves over this ratio
        ...
        internal static readonly int[] MoveImportance = new int[] {
...
#endif
        internal static int move_importance(int ply) { return MoveImportance[Math.Min(ply, 511)]; }

#if AGGR_INLINE
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
#endif
        internal static void pv_instability(int curChanges, int prevChanges)
        {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
#endif
        internal static int available_time() { return optimumSearchTime + unstablePVExtraTime; }

#if AGGR_INLINE
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
#endif
        internal static int maximum_time() { return maximumSearchTime; }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
#endif
        private static int remaining(TimeType T, int myTime, int movesToGo, int currentPly, int slowMover)
        {
            float TMaxRatio = (T == TimeTypeC.OptimumTime ? 1 : MaxRatio);
            float TStealRatio = (T == TimeTypeC.OptimumTime ? 0 : StealRatio);
            ...
            return (int)(Math.Floor(myTime * Math.Min(ratio1, ratio2)));
        }
    }
}
